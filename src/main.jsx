...
function AdminReviewPanel({ task }) {
  ...
  const review = async (status) => {
    ...
    if (profile.role === "manager") {
      await data.saveTask({ ...task, status, manager_remarks: managerRemarks, manager_reviewed_at: new Date().toISOString() });
      notify(`Task marked ${status}.`);
      return;
    }
    await data.saveTask({ ...task, status: status === "approved" ? "done" : status, admin_remarks: rating.remarks, admin_reviewed_at: new Date().toISOString(), final_forwarded_to_admin: status === "approved" ? false : task.final_forwarded_to_admin });
